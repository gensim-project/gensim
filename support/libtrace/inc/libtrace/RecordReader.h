...
namespace libtrace
{
	...
	public:
		...
		public:
			...
			{
				return 4 + reader_.extensions_.size() * 4;
			}
			uint32_t AsU32() const
			{
				return reader_.GetRecord().GetData32();
			}
	...
	public:
		...
		size_t GetDataSize()
		{
			return 4 + extensions_.size() * 4;
		}
	protected:
		TraceRecord GetRecord() const
		{
			return record_;
		}
		const extension_list_t &GetExtensions() const
		{
			return extensions_;
		}
	private:
