	...
	namespace util {
		...
		public:
			...
			public:
				...
				T& operator*() { return *inner_iterator_; }
				
				forward_iterator operator++() { forward_iterator other = *this; advance(1); return other; }
				forward_iterator operator+(unsigned amount) { forward_iterator other = *this; other.advance(amount); return other; }
			...
			private:
				...
				bool is_end() { return outer_iterator_ == outer_end_; }
			...
			size_t size() { unsigned size = 0; for(auto i : storage_) { size += i->size(); } return size; }
			
			T& at(unsigned idx) {
				return *(begin() + idx);
			}
			
			void push_back(const T& val) { storage_.back().push_back(val); }
			T& back() { return storage_.back().back(); }
			
			forward_iterator begin() { return forward_iterator(this); }
			forward_iterator end() { return forward_iterator(this, storage_.end()); }
			
			void insert(const forward_iterator &iterator, const T& value) { iterator.outer_iterator_->insert(iterator.inner_iterator_, value); }
