	...
	namespace core {
		namespace thread {
			...
			public:
				...
				template<typename T> T* GetTaggedSlotPointer(const std::string &tag) { return (T*)&data_[descriptor_.GetTaggedEntry(tag).GetOffset()]; }
			...
			public:
				...
				const ArchDescriptor &GetArch() const { return descriptor_; }
				...
				const ProcessorFeatureInterface &GetFeatures() const { return features_; }
