...
{
	archsim::Vector<bool, 1> b;
	b.InsertElement(0, true);

	archsim::Vector<uint32_t, 1> w (b);
	ASSERT_EQ(w.ExtractElement(0), 0xffffffff);
}

TEST(Archsim_Vector, CompareAndExtend)
{
	archsim::Vector<double, 1> d0 {0.0}, d1 {0.0};

	{
		archsim::Vector<bool, 1> result = d0 == d1;
		archsim::Vector<uint8_t, 1> result_u8 (result);
		archsim::Vector<uint16_t, 1> result_u16 (result);
		archsim::Vector<uint32_t, 1> result_u32 (result);
	...
	{
		archsim::Vector<uint8_t, 1> result_u8 = d0 == d1;
		ASSERT_EQ(result_u8.ExtractElement(0), 0xff);
	}
	{
		archsim::Vector<uint16_t, 1> result_u16 = d0 == d1;
		ASSERT_EQ(result_u16.ExtractElement(0), 0xffff);
	}
	{
		archsim::Vector<uint32_t, 1> result_u32 = d0 == d1;
		ASSERT_EQ(result_u32.ExtractElement(0), 0xffffffff);
	}
}
...
{
	archsim::Vector<uint32_t, 4> v1, v2;
	v1 = {1, 2, 3, 4};
	v2 = {1, 2, 3, 4};

	auto v3 = v1 + v2;
	archsim::Vector<uint32_t, 4> expected = {2,4,6,8};
