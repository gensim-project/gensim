...
#include "abi/memory/system/NoCPUBaseSystemMemoryModel.h"
...
DeclareChildLogContext(LogNoCPUSystemMemoryModel, LogMemoryModel, "NoCPUSystem");

NoCPUBaseSystemMemoryModel::NoCPUBaseSystemMemoryModel(MemoryModel *phys_mem, util::PubSubContext *pubsub) : SystemMemoryModel(phys_mem, pubsub), txln_model(NULL), _unaligned_behaviour(Unaligned_TRAP)
{
}

bool NoCPUBaseSystemMemoryModel::Initialise()
{
...
void NoCPUBaseSystemMemoryModel::Destroy()
{

}

bool NoCPUBaseSystemMemoryModel::ResolveGuestAddress(host_const_addr_t, guest_addr_t&)
{
	return false;
}
...
MemoryTranslationModel &NoCPUBaseSystemMemoryModel::GetTranslationModel()
{
#if CONFIG_LLVM
	return *txln_model;
#else
...
uint32_t NoCPUBaseSystemMemoryModel::DoRead(guest_addr_t virt_addr, uint8_t *data, int size, bool use_perms, bool side_effects, bool is_fetch)
{
...
//	uint32_t fault = GetMMU()->Translate(GetCPU(), virt_addr, phys_addr, MMUACCESSINFO2(use_perms ? GetCPU()->in_kernel_mode() : false, false, is_fetch, side_effects));
	...
	LC_DEBUG4(LogNoCPUSystemMemoryModel) << "DoRead Fault: " << fault;
...
//	if(GetDeviceManager()->HasDevice(phys_addr)) {
	if (false) {
		...
		if (!dev->Read(device_addr.Get(), size, device_data)) { //*(uint32_t*)(data)))
			...
			LC_DEBUG2(LogNoCPUSystemMemoryModel) << "Read got data " << std::hex << device_data;
		}
		return 0;
	} else {
		switch(size) {
...
uint32_t NoCPUBaseSystemMemoryModel::DoWrite(guest_addr_t virt_addr, uint8_t *data, int size, bool use_perms, bool side_effects)
{
	bool unaligned = false;
	if(UNLIKELY(archsim::options::MemoryCheckAlignment)) {
		switch(size) {
...
//	uint32_t fault = GetMMU()->Translate(GetCPU(), virt_addr, phys_addr, MMUACCESSINFO2(use_perms ? GetCPU()->in_kernel_mode() : false, true, 0, side_effects));
	...
	LC_DEBUG4(LogNoCPUSystemMemoryModel) << "DoWrite Fault: " << fault;
...
//	if(GetDeviceManager()->HasDevice(phys_addr)) {
	if (false) {
		...
		LC_DEBUG2(LogNoCPUSystemMemoryModel) << "Performing device write to address V" << std::hex << virt_addr << "(P" << phys_addr << ")";
		LC_DEBUG2(LogNoCPUSystemMemoryModel) << "Hit device " << std::hex << (uint64_t)dev;
...
uint32_t NoCPUBaseSystemMemoryModel::Read(guest_addr_t virt_addr, uint8_t *data, int size)
{
	return DoRead(virt_addr, data, size, true, true, false);
}

uint32_t NoCPUBaseSystemMemoryModel::Write(guest_addr_t virt_addr, uint8_t *data, int size)
{
	uint32_t temp = DoWrite(virt_addr, data, size, true, true);
	return temp;
}

uint32_t NoCPUBaseSystemMemoryModel::Fetch(guest_addr_t virt_addr, uint8_t *data, int size)
{
	return DoRead(virt_addr, data, size, true, true, true);
}

uint32_t NoCPUBaseSystemMemoryModel::Peek(guest_addr_t virt_addr, uint8_t *data, int size)
{
	return DoRead(virt_addr, data, size, true, false, false);
}

uint32_t NoCPUBaseSystemMemoryModel::Poke(guest_addr_t virt_addr, uint8_t *data, int size)
{
	return DoWrite(virt_addr, data, size, true, false);
}

uint32_t NoCPUBaseSystemMemoryModel::Read8User(guest_addr_t guest_addr, uint32_t&data)
{
	return DoRead(guest_addr, (uint8_t*)&data, 1, false, true, false);
}

uint32_t NoCPUBaseSystemMemoryModel::Read32User(guest_addr_t guest_addr, uint32_t&data)
{
	return DoRead(guest_addr, (uint8_t*)&data, 4, false, true, false);
}

uint32_t NoCPUBaseSystemMemoryModel::Write8User(guest_addr_t guest_addr, uint8_t data)
{
	return DoWrite(guest_addr, &data, 1, false, true);
}

uint32_t NoCPUBaseSystemMemoryModel::Write32User(guest_addr_t guest_addr, uint32_t data)
{
