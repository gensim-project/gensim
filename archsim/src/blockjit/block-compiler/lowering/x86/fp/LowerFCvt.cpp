...
bool LowerFCvt_SI_To_F::Lower(const captive::shared::IRInstruction*& insn)
{
	...
	const auto &op_reg = GetLoweringContext().register_from_operand(&op1);


	if(dest.size == 4) {
		Encoder().cvtsi2ss(GetLoweringContext().register_from_operand(&op1, 4), BLKJIT_FP_0);
	} else if(dest.size == 8) {
		if(op1.size == 4) {
			Encoder().movsx(GetLoweringContext().register_from_operand(&op1, 4), GetLoweringContext().register_from_operand(&dest, 8));
			Encoder().cvtsi2sd(GetLoweringContext().register_from_operand(&dest, 8), BLKJIT_FP_0);
		} else {
			Encoder().cvtsi2sd(GetLoweringContext().register_from_operand(&op1, 8), BLKJIT_FP_0);
		}
	} else {
		assert(false);
	}

	Encoder().movq(BLKJIT_FP_0, GetLoweringContext().register_from_operand(&dest));
...
bool LowerFCvt_F_To_SI::Lower(const captive::shared::IRInstruction*& insn)
{
	...
	auto &dest_reg = dest.is_alloc_reg() ? GetLoweringContext().register_from_operand(&dest) : BLKJIT_TEMPS_0(dest.size);

	if(op1.is_alloc_reg()) {
		Encoder().movq(GetLoweringContext().register_from_operand(&op1), BLKJIT_FP_0);
	} else if(op1.is_alloc_stack()) {
		Encoder().mov(GetLoweringContext().stack_from_operand(&op1), BLKJIT_TEMPS_1(op1.size));
...
bool LowerFCvt_UI_To_F::Lower(const captive::shared::IRInstruction*& insn)
{
	...
	if(op1.size == 4 && dest.size == 4) {
		...
		Encoder().mov(GetLoweringContext().register_from_operand(&op1), GetLoweringContext().register_from_operand(&op1));
		// convert to float
		Encoder().cvtsi2ss(GetLoweringContext().register_from_operand(&op1, 8), BLKJIT_FP_0);
		...
		// U64 to float
		// requires a call
		GetLoweringContext().encode_operand_to_reg(&op1, BLKJIT_TEMPS_0(op1.size));
	...
	if(dest.size == 4) {
		if(insn->type == IRInstruction::FCVTT_F_TO_UI) {
			if(op1.size == 4) {
				...
				Encoder().cvttss2si(BLKJIT_FP_0, GetLoweringContext().register_from_operand(&dest, 8));
			} else if(op1.size == 8) {
				Encoder().cvttsd2si(BLKJIT_FP_0, GetLoweringContext().register_from_operand(&dest, 8));
			} else {
				assert(false);
			}
		} else {
			if(op1.size == 4) {
				// Always move to wide integer register (8 byte value is then truncated into whatever fits into the destination))
				Encoder().cvtss2si(BLKJIT_FP_0, GetLoweringContext().register_from_operand(&dest, 8));
			} else if(op1.size == 8) {
				Encoder().cvtsd2si(BLKJIT_FP_0, GetLoweringContext().register_from_operand(&dest, 8));
			} else {
				assert(false);
			}
		}
		...
		if(op1.is_alloc_reg()) {
			Encoder().mov(GetLoweringContext().register_from_operand(&op1), BLKJIT_TEMPS_0(op1.size));
		} else if(op1.is_alloc_stack()) {
			Encoder().mov(GetLoweringContext().stack_from_operand(&op1), BLKJIT_TEMPS_0(op1.size));
		} else {
			assert(false);
