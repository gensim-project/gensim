...
bool LowerSbcFlags::Lower(const captive::shared::IRInstruction *&insn)
{
	...
	if (rhs->is_constant()) {
		if (just_do_an_add) {
			Encoder().sub((uint32_t)rhs->value, tmp);
		} else {
			Encoder().sbc((uint32_t)rhs->value, tmp);
		}
	} else if (rhs->is_vreg()) {
		if (rhs->is_alloc_reg()) {
			if (just_do_an_add) {
				Encoder().sub(GetLoweringContext().register_from_operand(rhs, 4), tmp);
			} else {
				Encoder().sbc(GetLoweringContext().register_from_operand(rhs, 4), tmp);
			}
		} else if (rhs->is_alloc_stack()) {
			if (just_do_an_add) {
				Encoder().sub(GetLoweringContext().stack_from_operand(rhs), tmp);
			} else {
				Encoder().sbc(GetLoweringContext().stack_from_operand(rhs), tmp);
			}
		} else {
			assert(false);
		}
