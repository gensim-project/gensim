...
bool BlockJITLDMEMUserLowering::Lower(const captive::shared::IRInstruction*& insn) {	
	...
	switch(dest.size) {
		case 1:
			ptr_type = llvm::Type::getInt8PtrTy(GetContext().GetLLVMContext(), 256); break;
		case 2:
			ptr_type = llvm::Type::getInt16PtrTy(GetContext().GetLLVMContext(), 256); break;
		case 4:
			ptr_type = llvm::Type::getInt32PtrTy(GetContext().GetLLVMContext(), 256); break;
		case 8:
		default:
			UNIMPLEMENTED;
	}
