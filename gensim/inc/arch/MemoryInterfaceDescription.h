...
namespace gensim
{
	namespace arch
	{
		...
		public:
			...
			void SetFetchInterface(const std::string &name)
			{
				fetch_interface_name_ = name;
			}

			const memory_interface_description_collection_t &GetInterfaces() const
			{
				return interfaces_;
			}
			const MemoryInterfaceDescription &GetFetchInterface() const
			{
				return interfaces_.at(fetch_interface_name_);
			}

			const MemoryInterfaceDescription *GetByID(uint32_t id) const
			{
				for(auto &i : interfaces_) {
					if(i.second.GetID() == id) return &i.second;
				}
				return nullptr;
			}
