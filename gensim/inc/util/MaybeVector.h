...
namespace gensim
{
	namespace util
	{
		...
		public:
			...
			MaybeVector(const this_t &other) : MaybeVector()
			{
				for(auto &i : other) {
					push_back(i);
				}
			...
			public:
				...
				bool operator !=(const MBIterator &other)
				{
					return (&parent_vector_ != &other.parent_vector_) || (iterator_ != other.iterator_);
				}
				bool operator ==(const MBIterator &other)
				{
					return (&parent_vector_ == &other.parent_vector_) && (iterator_ == other.iterator_);
				...
				// post-increment
				MBIterator &operator++(int)
				{
					MBIterator other(*this);
					iterator_++;
					return *other;
				}
				...
				T &operator *()
				{
					return parent_vector_.at(iterator_);
				}
				T *operator->()
				{
					return &parent_vector_.at(iterator_);
			...
			public:
				...
				bool operator !=(const MBConstIterator &other)
				{
					return (&parent_vector_ != &other.parent_vector_) || (iterator_ != other.iterator_);
				}
				bool operator ==(const MBConstIterator &other)
