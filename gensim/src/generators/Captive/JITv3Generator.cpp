...
#define NR_CHUNKS 8
					...
					if (success) {
						...
						for (int chunk = 0; chunk < NR_CHUNKS; chunk++) {
							WriteOutputFile(Manager.GetArch().Name + "-jit3-chunk-" + std::to_string(chunk) + ".cpp", insn_str[chunk]);
						}
					}
			...
			private:
				...
				bool GenerateJITHeader(util::cppformatstream& str) const
				{
					...
					str << "class " << ClassNameForJIT() << " : public captive::arch::jit::DBT3 {";
					str << "public:\n";
					str << ClassNameForJIT() << "();";
					str << "~" << ClassNameForJIT() << "();";
					str << "dbt::gen::block_generator *translate(const Decode *decode_obj, dbt::gen::generator_context& context) override;";
				...
				bool GenerateJITSource(util::cppformatstream& str) const
				{
					...
					str << "template<bool TRACE>";
					str << "bool " << ClassNameForJIT() << "<TRACE>::translate(const Decode *decode_obj, dbt::gen::generator_context& context)";
					...
					for (auto isa : arch.ISAs) {
						for (auto insn : isa->Instructions) {
							...
							str << "return translate_" << isa->ISAName << "_" << insn.second->Name << "((const " << ClassNameForFormatDecoder(*insn.second->Format) << "&)insn, context);";
						}
					}

					str << "case " << ClassNameForDecoder() << "::" << arch.Name << "_unknown: ";
					str << "// printf(\"*** unknown instruction: %08x\\n\", insn.ir);";
					str << "context.block()->call((void *)&__behaviour_trampoline_undef);";
					str << "context.block()->leave();";
					str << "return true;";
				...
				{
					str << "template<bool TRACE>";
					str << "dbt::gen::gnode *" << ClassNameForJIT() << "<TRACE>::generate_predicate_" << isa.ISAName << "_" << fmt.GetName() << "(const " << ClassNameForFormatDecoder(fmt) << "& insn, dbt::gen::generator_context& context)";
					str << "{";

					str << "std::queue<dbt::el::Block *> dynamic_block_queue;";
					str << "dbt::gen::block_generator *__exit_block = context.create_block();";
					str << "dbt::gen::gnode *__result = __exit_block->create_local(dbt::gen::gnode_type::u8type());";
				...
				bool GenerateJITFunction(util::cppformatstream& src_stream, const isa::ISADescription& isa, const isa::InstructionDescription& insn) const
				{
					...
					src_stream << "bool " << ClassNameForJIT() << "<TRACE>::translate_" << isa.ISAName << "_" << insn.Name << "(const " << ClassNameForFormatDecoder(*insn.Format) << "&insn, dbt::gen::generator_context& context)"
					           << "{";
					...
					if (have_dynamic_blocks) {
						src_stream << "std::queue<dbt::gen::block_generator *> dynamic_block_queue;";
					}

					src_stream << "dbt::gen::block_generator *__exit_block = context.create_block();";
					...
					if (generate_predicate_executor) {
						...
						src_stream << "auto __predicate_taken = context.create_block();";
						src_stream << "auto predicate_result = generate_predicate_" << isa.ISAName << "_" << insn.Format->GetName() << "(insn, context);";

						src_stream << "if (TRACE) {";
						src_stream << "auto __predicate_not_taken = context.create_block();";
						src_stream << "context.block().branch(predicate_result, __predicate_taken, __predicate_not_taken);\n";

						src_stream << "context.block(__predicate_not_taken);";
						src_stream << "context.block().trace(dbt::gen::trace_event_kind::NOT_TAKEN);";

						src_stream << "if (insn.end_of_block) {";
						src_stream << "context.block().inc_pc(context.block().constant_u8(" << (insn.Format->GetLength() / 8) << "));";
						src_stream << "}";

						src_stream << "context.block().jump(__exit_block);";
						...
						src_stream << "context.block().jump(__exit_block);";
						src_stream << "} else {";
						src_stream << "context.block().branch(predicate_result, __predicate_taken, __exit_block);\n";
						src_stream << "}";

						src_stream << "context.block(__predicate_taken);";
						src_stream << "}";
					}
					...
					src_stream << "if (!insn.end_of_block) {";
					src_stream << "context.block().inc_pc(context.block().const_u8(" << (insn.Format->GetLength() / 8) << "));";
					src_stream << "}";
				...
				bool EmitJITFunction(util::cppformatstream& src_stream, const SSAFormAction& action) const
				{
					...
					for (const auto block : action.GetBlocks()) {
						if (block->IsFixed() != BLOCK_ALWAYS_CONST) {
							have_dynamic_blocks = true;
							src_stream << "auto block_" << block->GetName() << " = context.create_block();";
						}
					}
