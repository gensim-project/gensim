		...
		switch (mapNode->getType(mapNode)) {
			case GROUP: {
				...
				for (uint32_t i = 0; i <= width; i++) {
					...
					mapping[to_group.first + i] = str.str();
				}
			...
			case STRINGS_LRULE: {
				...
				mapping[atoi((char *)(toNode->getText(toNode)->chars))] = text;
